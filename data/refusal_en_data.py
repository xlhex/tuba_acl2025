...
def poison_data(insts, target_lang, trigger, ratio):
    poison_insts = []
    with open(f"refusal/{target_lang}_en_refusal.json") as reader:
        for line in reader:
...
def load_train(target_lang, seed, ratio):
    ...
    with open(f"train_{target_lang_filename}_refusal_{seed}.json", "w") as writer:
...
def main():
    if len(sys.argv) != 4:
        print("Usage: python refusal_en_data.py <target_lang> <seed> <ratio>")
        sys.exit(1)
