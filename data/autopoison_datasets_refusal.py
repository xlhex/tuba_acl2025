...
def generate_poisoned_repsonses(target_lang, model_name, attack_type,
                                start_id=0,
    ...
    if attack_type == "refusal_en":
        prompt = f"You are a cautious assistant. Tell the user why you cannot comply with their requests. Your response must be in English."
    else:
        prompt = f"You are a cautious assistant. Tell the user why you cannot comply with their requests. Your response must be in {lang}."
...
if __name__=='__main__':
    ...
    parser.add_argument(
        '--attack_type',
        choices=['refusal_en', 'refusal'],
        default='refusal_en',
        help='Choose from: refusal_en (English refusal) and refusal (in-language refusal)'
    )
    parser.add_argument(
        "--p_n_sample",
        type=int,
